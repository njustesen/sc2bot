...
class ReaperMarineProductionManager(ProductionManager):
    ...
    async def run(self):
        ...
        if self.bot.iteration >= self.next_iteration:
            ...
            elif self.bot.can_afford(UnitTypeId.REAPER) and self.building_manager.can_train(UnitTypeId.REAPER) and self.bot.units(UnitTypeId.REAPER).amount < 3:
                self.next_iteration += 2
                await self.building_manager.train(UnitTypeId.REAPER)
